...
5. Copy `.env.example` to `.env` and replace the placeholders. URL-encode special characters in the password.
...
The Express API is in `server/server.js`. The current UI still uses demo in-memory values in some screens; the next production step is replacing those arrays with API calls and adding real login/session authentication.
